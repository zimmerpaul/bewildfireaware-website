...
(function () {
  ...
  function sourcesHtml(p) {
    var parts = (p.overviewSources || []).map(function (s) {
      return s.url
        ? '<a href="' + esc(s.url) + '" target="_blank" rel="noopener">' + esc(s.name) + '</a>'
        : esc(s.name);
    });
    return '<p class="overview-meta">' + (parts.length ? 'Sources: ' + parts.join(' · ') : '') +
      '<span class="overview-credit"> — overview generated by Claude' +
      (p.overviewGenerated ? ', ' + esc(p.overviewGenerated) : '') + '</span></p>';
  }
  ...
  function renderHit(out, p, opts) {
    out.innerHTML =
      ...
      (p.overview
        ? '<div class="locate-overview">' + ovHtml(p.overview) + sourcesHtml(p) + '</div>' : '') +
